class wshnjs_fs_stats {
    ...
    constructor(path: string) {
        this.path = path;
        if (wshnjs_fs_stats.wsh_fs.FileExists(this.path)) {
            var file: any = wshnjs_fs_stats.wsh_fs.GetFile(this.path);
            this.dev = -1;
            this.ino = -1;
            this.mode = -1;
            this.nlink = -1;
    ...
    isDirectory(): boolean {
        if (wshnjs_fs_stats.wsh_fs.FolderExists(this.path)) {
            return true;
        } else {
            return false;
        }
...
class wshnjs_fs {
    ...
    stat(path: string, callback?: (err: any, stats: wshnjs_fs_stats) => any): void {
    ...
    lstat(path: string, callback?: (err: any, stats: wshnjs_fs_stats) => any): void {
    ...
    statSync(path: string): wshnjs_fs_stats {
        return new wshnjs_fs_stats(path);
    }

    lstatSync(path: string): wshnjs_fs_stats {
    ...
    readlink(path: string, callback?: (err: any, linkString: string) => any): void {
        var linkString = this.readlinkSync(path);
        if (callback) {
    ...
    realpath(path: string, callback?: (err: any, resolvedPath: string) => any): void {
        var resolvedPath = this.realpathSync(path);
        if (callback) {
    ...
    unlinkSync(path: string): void {
        if (wshnjs_fs.wsh_fs.FileExists(path)) {
            wshnjs_fs.wsh_fs.DeleteFile(path);
        }
    }
    ...
    rmdirSync(path: string): void {
        if (wshnjs_fs.wsh_fs.FolderExists(path)) {
            wshnjs_fs.wsh_fs.DeleteFolder(path);
        }
    }
    ...
    mkdirSync(path: string, mode?: string): void {
        wshnjs_fs.wsh_fs.CreateFolder(path);
    }

    readdir(path: string, callback?: (err: any, files: string[]) => void): void {
        var files = this.readdirSync(path);
        if (callback) {
    ...
    open(path: string, flags: string, callback?: (err: any, fd: number) => any): void {
        var fd = this.openSync(path, flags);
        if (callback) {
    ...
    readFile(filename: string, encoding: string, callback: (err: any, data: string) => void): void {
        var data = this.readFileSync(filename, encoding);
        if (callback) {
    ...
    readFileSync(filename: string, encoding: string): string {
        var data: string = '';
        if (wshnjs_child_process_executor.wsh_fs.GetFile(filename).Size > 0) {
            var reader: any = wshnjs_fs.wsh_fs.OpenTextFile(filename, ForReading, false);
            data = reader.ReadAll();
            reader.Close();
        }
        return data;
    ...
    writeFileSync(filename: string, data: any, options?: { encoding?: string; mode?: string; flag?: string; }): void {
        var writer: any = wshnjs_fs.wsh_fs.OpenTextFile(filename, ForWriting, true);
        writer.Write(data);
        writer.Close();
    }
    ...
    appendFileSync(filename: string, data: any, options?: { encoding?: string; mode?: string; flag?: string; }): void {
        var writer: any = wshnjs_fs.wsh_fs.OpenTextFile(filename, ForAppending, true);
        writer.Write(data);
        writer.Close();
    }
    ...
    existsSync(path: string): boolean {
        if (wshnjs_fs.wsh_fs.FileExists(path)) {
            return true;
        } else if (wshnjs_fs.wsh_fs.FolderExists(path)) {
            return true;
        } else {
            return false;
        }
